...
const RenewCertificate = async ({ params }: { params: { id: string } }) => {
  const renewCertificate = async (formData: FormData) => {
    "use server";
    ...
    const response = await fetch(`${backend}/api/certificates/${params.id}/renew`, {
      method: "POST",
      cache: "no-store",
      headers: {
  ...
  if (response.status == 200) {
    ...
    return (
      <div>
        ...
        <CertificateForm action={renewCertificate} certificate={certificate} />
      </div>
    );
  }
